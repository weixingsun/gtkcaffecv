...
void  Classifier::load_image(cv::Mat &img)
{
    ...
    //std::unique_ptr<std::thread>   ptr_thread(new std::thread(&Classifier::fun, this));
    //mptr_thread = std::move(ptr_thread);
    //mptr_thread.reset(new std::thread(&Classifier::fun, this));

    //mp_thread = new std::thread(&Classifier::fun, this);
}
